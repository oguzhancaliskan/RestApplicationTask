...
public class UploadService {
		...
		try {
			...
			for (String line: allLines) {
				...
				if (st.hasMoreTokens()) {
					...
					if (primaryKey.contentEquals(" ") || recordRepo.containsKey(primaryKey))  //validate primary key
	...
	public ResponseEntity<Object> getRecordService(String id) {
		if(recordRepo.get(id) == null) 
			return new ResponseEntity<Object>("Unable to get record " + id, HttpStatus.NOT_FOUND);
	...
	public ResponseEntity<Object> deleteRecord(String id) { 
		if(recordRepo.get(id) == null) 
			return new ResponseEntity<Object>("Unable to delete record " + id, HttpStatus.NOT_FOUND);
